...
export const math_3: TestItem[] = [
	...
	new TestItemSelfCheck(14, DBc("Целые числа"), DBc().title("Целые числа").text("0, ±1, ±2, ±3...", "Множество целых чисел обозначается 'Z")),
	...
	new TestItemSelfCheck(21, DBc("Метод интервалов"), DBc()
		...
		.ol(
			"Найдём нули числителя и знаменателя: a, b, c, d",
			"Отметим их на оси x",
			"Определяем знак функции на каждом из промежутков между точками. Для определения знака берем любую точку, принадлежащую интервалу",
			"Записываем в ответ подходящие по знаку интервалы"
		)
		.title("Пример")
	...
	new TestItemSelfCheck(26,
		DBc()
			...
			.formula(createFormulas("@{@log}_ab = ?"))
		, DBc()
			.text("Основные формулы для логарифмов", "")
